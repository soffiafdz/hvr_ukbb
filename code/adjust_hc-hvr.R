...
cnn.l.dt[, Z := scale(value), by = variable]
...
if (all(!REMATCH, file.exists(fpath))) {
  ...
  # Distances
  dist.lst  <- list(
    OFFSET  = 1,   # Time between sessions: M & F: 1.2 (1-8)
    AGE     = 1,   # Age: F:7 (45-82) & M:7.8 (44-81) (CS)
    ICC     = 25
  )  # ICC: F:107 (1018-2524) & M:126 (1289-2392) (CS)

  # Data setup
  # Sex-stratified input
  # Cross-sectional data
  crs_f.dt  <- crs.dt[SEX %like% "F", .(AGE, ICC, MATCHED = F), by = EID]
  crs_m.dt  <- crs.dt[SEX %like% "M", .(AGE, ICC, MATCHED = F), by = EID]

  # Longitudinal data
  # Use first instance data and time offset (years) between instances
  lng_b.dt  <- lng.dt[
    ,
    .(OFF = diff(AGE), MATCHED = FALSE),
    by = EID
  ][
    lng.dt["ses-2", on = "INST"]
  ]
  lng_f.dt  <- lng_b.dt[SEX %like% "F", .(OFF, AGE, ICC, MATCHED), by = EID]
  lng_m.dt  <- lng_b.dt[SEX %like% "M", .(OFF, AGE, ICC, MATCHED), by = EID]
  ...
  # Progress bar
  pb <- progress_bar$new(
    format = "Matching | :what [:bar] :current/:total",
  ...
  for (m.i in lng_m.dt[, .I]) {
    ...
    if (length(match.i) > 0) {
      ...
      match.row <- lng_f.dt[
        match.i,
        .(EID_f = EID, AGE, OFF, ICC, ICC_f = ICC)
      ][
        lng_m.dt[m.i],
        on = "ICC",
        .(
...
for (i in seq_along(hcvc.lst)) {
  ...
  for (j in seq_along(hcvc.lst[[i]])) {
    ...
    hcvc.lst[[i]][[j]]$ALL <- merge(
      list(crs.dt, lng.dt)[[j]][, -"ICC"],
      hcvc.dt[names(hcvc.lst)[i], on = "SEGM", -"SEGM"],
      all = FALSE
    )
    hcvc.lst[[i]][[j]]$MTCH <- match.lst[[j]][, .(MATCH = ID, EID_m, EID_f)] |>
...
hc_hvr.lst  <-   hcvc.lst |>
lapply(function(segm_lvl) lapply(
  segm_lvl, function(dsgn_lvl) lapply(
    dsgn_lvl, function(match_lvl) {
      # Mean ICC
      match_lvl[, ICC_mean := mean(ICC)]
      # CC ~ ICC Regression slope (per ROI)
      match_lvl[, B := summary(lm(CC ~ ICC))[[4]][2], ROI]
...
### OUT
outrds      <-
  c("sex-age-icc_matching", "hc-hvr_adj") |>
