...
metadata.dt <- fread(fpaths[1],
                     select = c(1:2, 24, 6:7, 10:14, 17:21),
                     col.names = c("FIELD",
                                   "TITLE",
                                   "N",
                                   "TYPE_value",
                                   # 11—integer, 21—single-choice,
                                   # 22—multi-choice, #31—real, 41—string,
                                   # 51—date, 61—time, 101—compound, 201—blob
                                   "TYPE_base",
                                   # 0—value, 11—int, 31—real,
                                   # 41—string, 51—date
...
cols.dt     <- c(## Demographics
                 ...
                 SYM_corr             = 23323,  # Correct matches
                 SYM_corr_onl         = 20159,  # Correct matches (online)
                 SYM_try              = 23324,  # Attempted matches
  ...
  if (name.col %like% "ILL|ICD") {
    if (instanced) {
      for (j in metadata.dt[i, seq(INSTANCE_min, INSTANCE_max)]) {
        ...
        DT[, (out.col) := do.call(paste, c(.SD, sep = ",")),
           .SDcols = src.cols] |> suppressWarnings()
...
## OUT
outfiles <- here("data/rds", c("metadata.rds",
                               "notes.rds",
